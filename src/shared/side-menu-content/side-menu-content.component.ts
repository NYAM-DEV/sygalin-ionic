...
class InnerMenuOptionModel {
	...
	public baseSuboptionsCount: number;
	public subOptions: Array<InnerMenuOptionModel>;
	public allowed: Array<any>;
	...
	public static fromMenuOptionModel(option: SideMenuOption, parent?: InnerMenuOptionModel): InnerMenuOptionModel {
		...
		if (option.suboptions) {
			...
			innerMenuOptionModel.baseSuboptionsCount=innerMenuOptionModel.suboptionsCount;
			//innerMenuOptionModel.divHeight = (this.optionHeight + 1) * innerMenuOptionModel.suboptionsCount;
...
export class SideMenuContentComponent {
	...
	private hasParent(option)
	{
		return option.parent!==null;
	}

	private hasGParent(option)
	{
		return this.hasParent(option) && option.parent.parent!=null;
	}
	...
	public toggleItemOptions(targetOption: InnerMenuOptionModel): void {
		...
		if (this.menuSettings.accordionMode) {
			if (!this.hasParent(targetOption)){
				...
				this.collapsableItems.forEach(option => {
					if (option.id!==targetOption.parent.id)
					{
						option.expanded=false;
					}
				});
				targetOption.parent.subOptions.forEach(sub=>{
					if (sub.id!==targetOption.id)
					{
						sub.expanded=false;
					}
				});
				targetOption.parent.suboptionsCount=targetOption.parent.baseSuboptionsCount;
				targetOption.expanded=!targetOption.expanded;
				if (targetOption.expanded){
					targetOption.parent.suboptionsCount=targetOption.parent.baseSuboptionsCount+targetOption.suboptionsCount;
				}
			}
		} else {
			targetOption.expanded=!targetOption.expanded;
			if (targetOption.expanded && this.hasParent(targetOption)){
				targetOption.parent.suboptionsCount+=targetOption.suboptionsCount;
			}
			if (!targetOption.expanded && this.hasParent(targetOption)) {
				targetOption.parent.suboptionsCount-=targetOption.suboptionsCount;
			}
		}
		console.log("OPTIONS: ", this.collapsableItems);
		//targetOption.expanded=!targetOption.expanded;
